...
* :bumping - if the class as a lock_column set (ActiveRecord::Locking::Optimistic)
  then the instance will be touched, bumping the lock_version and checking for stale
  data.

Note, during a mediated_transaction, the lock_column will only update during the 
